...
async function initUserSession() {
  try {
    ...
    console.warn("Could not fetch current user, using default");
  }
}
...
function updateUserInterfaceElements() {
  ...
  if (role) role.textContent = state.currentUser.role.toUpperCase();
  if (curr) curr.textContent = state.currentUser.home_currency || 'USD';

  // Toggle Persona button active highlight
...
async function switchDemoUser(userId) {
  ...
  showToast(`Switched user persona to ${state.currentUser.name}`);
  handleHashChange();
}
...
function switchAuthTab(mode) {
  ...
  document.getElementById('auth-group-name').style.display = mode === 'signup' ? 'block' : 'none';
  document.getElementById('auth-submit-btn').textContent = mode === 'signup' ? 'Create Account' : 'Sign In';
  document.getElementById('auth-modal-title').textContent = mode === 'signup' ? 'Create GlobeTrotter Account' : 'Sign In to GlobeTrotter';
}
