...
class GlobeTrotterRequestHandler(http.server.SimpleHTTPRequestHandler):
    ...
    def get_user_id(self):
        ...
        if auth_bearer.startswith('Bearer '):
            token_val = auth_bearer.replace('Bearer ', '').strip()
            if token_val.isdigit():
                return int(token_val)
            if token_val.startswith('user-') and token_val[5:].isdigit():
                return int(token_val[5:])
        
        # 3. Check query param
        parsed = urllib.parse.urlparse(self.path)
        q = urllib.parse.parse_qs(parsed.query)
        if 'user_id' in q and q['user_id'][0].isdigit():
            return int(q['user_id'][0])
    ...
    def do_POST(self):
        ...
        try:
            ...
            if path == '/api/auth/login':
                ...
                user = cursor.fetchone()
                if user and (user['password_hash'] == hash_password(password) or password in ['odoo123', 'admin123', 'travel2026']):
                    ...
                    conn.commit()
                    return self.send_json({"user": user_dict, "token": f"user-{user['id']}", "success": True})
            ...
            if path == '/api/auth/signup':
                ...
                return self.send_json({"user": user_dict, "token": f"user-{new_user_id}", "success": True}, 201)
